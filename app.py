...
# 0. 全局配置与虚拟数据库初始化
# ==========================================
st.set_page_config(page_title="全链路跟单系统(终极版)", page_icon="🏭", layout="wide", initial_sidebar_state="expanded")
...
BOM_MASTER = {
    ...
    "精密冲压端子-B型": {"MAT-002": 1, "MAT-004": 0.1} # 10个端子用1个盒子
}
...
if 'orders' not in st.session_state:
    st.session_state.orders = pd.DataFrame({
        ...
        "收款情况": ["未收款"], #
        "异常说明": ["无"], 
        "当前状态": ["新建"], # 新建 → 备料中 → 可生产 → 生产中 → 待出货 → 已出货
        "物流公司": [""],
        "物流运单": [""]
    })
...
def run_mrp(order_id):
    ...
    for mat_code, unit_qty in bom.items():
        ...
        if shortage > 0:
            ...
            st.session_state.inventory.at[idx, "预留量"] += avail_qty # 把能用的全预留了
        else:
            # 库存够，直接预留
            idx = st.session_state.inventory.index[st.session_state.inventory["物料编码"] == mat_code].tolist()[0]
...
# 系统 UI 与 菜单
# ==========================================
menu = st.sidebar.radio("核心业务域", ["1. 首页看板", "2. 销售与订单", "3. 计划与采购", "4. 仓储物流", "5. 生产车间"])
...
if menu == "1. 首页看板":
    ...
    col1.metric("今日新增订单", len(st.session_state.orders[st.session_state.orders["当前状态"] == "新建"]))
...
# --- 2. 销售与订单 ---
    ...
    tab1, tab2 = st.tabs(["📝 录入新订单", "🔍 订单详情(一页到底)"])
    ...
    with tab2: #
        ...
        if sel_order:
            ...
            with dt3: #
                ...
                st.write(f"**预计发货日 (ETA):** {order_data['预计发货日']}")
    ...
    with tab1: #
        ...
        if not new_orders.empty:
            ...
            if st.button("执行计算 (算缺料并排产)", type="primary"):
                ...
                st.success(f"计算完成！物料状态：{status}。最晚到料日：{latest_arr}，系统推算 ETA：{eta}")
                st.rerun()
        else:
            st.info("无待计算订单")
...
# --- 5. 生产车间 ---
    ...
    if not prod_orders.empty:
        ...
        with st.form("prod_report"):
            ...
            abnormal = st.selectbox("异常提报", ["无", "停机", "缺人", "设备故障", "质量异常"]) #
