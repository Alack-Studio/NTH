...
# ==========================================
# 0. 全局配置与 Supabase 云端数据库初始化
# ==========================================
st.set_page_config(page_title="手搓跟单系统(全量版)", page_icon="🏭", layout="wide", initial_sidebar_state="expanded")

# --- 数据库连接 ---
SUPABASE_URL = "https://lnceopnqlvyyoahzpldw.supabase.co"
SUPABASE_KEY = "sb_publishable_OTdaRfwSeqMoPj6ZaVRK9w_J37eW7Ku"
...
# --- 核心函数：从云端拉取数据 ---
def fetch_data(table_name, default_df):
    try:
        ...
        if response.data:
            ...
            for col in df.columns:
                if '日' in col or '交期' in col:
                    df[col] = pd.to_datetime(df[col]).dt.date
            return df
        else:
            # 如果云端为空，写入默认数据
            insert_data = default_df.copy()
            for col in insert_data.select_dtypes(include=['datetime64[ns]', 'object']).columns:
                if '日' in col or '交期' in col:
                    insert_data[col] = insert_data[col].astype(str)
            supabase.table(table_name).insert(insert_data.to_dict(orient="records")).execute()
            return default_df
    except Exception as e:
        st.warning(f"读取表 {table_name} 时发生异常: {e}")
        return default_df

# --- 核心函数：一键保存全量数据到云端 ---
def sync_to_cloud():
    """将本地最新的 DataFrame 覆盖到云端"""
    tables = ['materials', 'inventory', 'products', 'orders', 'purchases']
    for tb in tables:
        df = st.session_state[tb]
        if not df.empty:
            df_copy = df.copy()
            # 必须把日期转换为字符串，否则云端 API 无法解析 JSON
            for col in df_copy.columns:
                if '日' in col or '交期' in col:
                    df_copy[col] = df_copy[col].astype(str)
            
            # 暴力替换法：先清空云端当前表，再全量插入新数据（适合第一阶段最小可用MVP）
            supabase.table(tb).delete().neq("id", -1).execute() 
            supabase.table(tb).insert(df_copy.to_dict(orient="records")).execute()
    st.success("☁️ 所有数据已成功同步至 Supabase 云端数据库！刷新页面数据不再丢失。")

# ==========================================
# 1. 初始化基础数据 (云端拉取 or 兜底数据)
# ==========================================
if 'materials' not in st.session_state:
    default_mat = pd.DataFrame({"物料编码": ["MAT-001", "MAT-002", "MAT-003", "MAT-004"], "物料名称": ["阻燃外壳", "纯铜插针", "控制芯片", "包装彩盒"], "采购周期(天)": [3, 5, 7, 2], "最小采购量": [1000, 5000, 500, 1000]})
    st.session_state.materials = fetch_data('materials', default_mat)

if 'inventory' not in st.session_state:
    default_inv = pd.DataFrame({"物料编码": ["MAT-001", "MAT-002", "MAT-003", "MAT-004"], "现存量": [5000, 10000, 200, 1000], "预留量": [0, 0, 0, 0], "安全库存": [500, 1000, 100, 200]})
    st.session_state.inventory = fetch_data('inventory', default_inv)

if 'products' not in st.session_state:
    default_prod = pd.DataFrame({"产品规格": ["漏电保护插头-标准款", "精密冲压端子-B型"], "标准日产能": [200, 1000], "包装缓冲天数": [1, 1]})
    st.session_state.products = fetch_data('products', default_prod)

if 'orders' not in st.session_state:
    default_ord = pd.DataFrame(columns=["订单编号", "客户名称", "产品规格", "订单数量", "已完工数", "承诺交期", "预计发货日", "最晚到料日", "收款情况", "异常说明", "当前状态", "物流公司", "物流运单"])
    st.session_state.orders = fetch_data('orders', default_ord)

if 'purchases' not in st.session_state:
    default_pur = pd.DataFrame(columns=["采购单号", "关联订单", "物料编码", "采购数量", "承诺到货日", "实际到货日", "状态"])
    st.session_state.purchases = fetch_data('purchases', default_pur)
...
# --- 侧边栏云端同步区 ---
st.sidebar.markdown("---")
if st.sidebar.button("☁️ 手动同步数据至云端", type="primary"):
    with st.spinner("正在将数据写入 Supabase..."):
        sync_to_cloud()
...
if menu == "1. 首页看板":
    ...
    with risk_col1:
        st.error("**到料超期预警**") 
        # 筛选今天之前承诺到货，但仍然是采购中的单据
        overdue_pos = st.session_state.purchases[
            (pd.to_datetime(st.session_state.purchases["承诺到货日"]).dt.date < date.today()) & 
            (st.session_state.purchases["状态"] == "采购中")
        ]
    ...
    with risk_col2:
        ...
        risk_orders = st.session_state.orders[
            (st.session_state.orders["预计发货日"].notna()) & 
            (pd.to_datetime(st.session_state.orders["预计发货日"]).dt.date > pd.to_datetime(st.session_state.orders["承诺交期"]).dt.date)
        ]
...
# --- 2. 销售与订单 ---
    ...
    with tab1:
        with st.form("new_order"):
            ...
            if st.form_submit_button("提交订单", type="primary"):
                ...
                st.success(f"订单 {oid} 创建成功！(记得点击左侧栏同步到云端保存)")
    ...
    with tab2:
        ...
        def check_risk(row):
            ...
            # 日期比较前先统一格式
            if pd.to_datetime(row["预计发货日"]).date() > pd.to_datetime(row["承诺交期"]).date(): return "🔴 延期风险"
            return "🟢 正常"
    ...
    with tab3: 
        if not st.session_state.orders.empty:
            ...
            if sel_order:
                ...
                with dt3: 
                    progress_val = order_data['已完工数'] / order_data['订单数量'] if order_data['订单数量']>0 else 0
...
# --- 3. 计划与采购 ---
    ...
    with tab2: 
        ...
        st.dataframe(po_df)
        if not po_df.empty:
            pending_po = po_df[po_df["状态"]=="采购中"]
            if not pending_po.empty:
                po_id = st.selectbox("选择采购单更新状态", pending_po["采购单号"])
...
# --- 5. 生产车间 ---
    ...
    prod_orders = st.session_state.orders[st.session_state.orders["当前状态"].isin(["可生产", "备料中", "生产中"])]
...
# --- 6. 基础数据 ---
    ...
    if st.button("保存基础数据变更", type="primary"):
        ...
        st.success("产能基准已在本地更新，请点击左侧栏同步至云端。")
